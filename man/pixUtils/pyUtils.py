...
def prr(name, img):
    ...
    try:
        data = f"{name}:\t\t{dtype:25s} [{img.min():6.3f} {img.max():6.3f}] {str(list(img.shape)):30s} [{nUnique} {unique[::step]}]"
    except:
        data = f"{name}:\t\t{dtype:25s} [{np.min(img):6.3f} {np.max(img):6.3f}] {str(list(img.shape)):30s} [{nUnique} {unique[::step]}]"
    print(data.strip())
...
def downloadDB(url, des, rename='', unzip=True):
    des = getPath(des)
    ...
    if rename:
        returnData = dirop(returnData, mvDir=dirname(returnData), desName=rename)
    if unzip:
        try:
            print("unzip dataset")
...
def getEc2PushPull(userName, pemPath):
    ...
    def ec2pull(remoteSrc, desDir):
        remoteSrc, desDir = remoteSrc.strip(), desDir.strip()
        cmd = f'''
        mkdir -p {desDir}
        {scp} -r {ec2}:{remoteSrc} {desDir}/
